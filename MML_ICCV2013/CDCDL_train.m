...
% Xp    ,Xs    : Image Data Pairs of two domains  d1xn, d2xn
% C            : Keywords for Images cxn
% Dp    ,Ds    : Initial Dictionaries d1xk1, d2xk2
...
function [Alphap, Alphas, Xp, Xs, Dp, Ds, Wp, Ws, Up, Us, f] = CDCDL_train(Alphap, Alphas, Xp, Xs, C, Dp, Ds, Wp, Ws, par)
...
%     Alphas = mexLasso([Xs; sqrtmu * Us * C], [Ds; sqrtmu * eye(size(Ws, 1))], param);
%     Alphap = mexLasso([Xp; sqrtmu * Up * C], [Dp; sqrtmu * eye(size(Wp, 1))], param);
    dictSize = par.K;
...
%     ts = inv(Up)*Alphap; % �˴���inv(Up) ���������е�Up, ��������¹�����ʵ����������е�Up^{-1}���ģ�
%     tp = inv(Us)*Alphas;
...
%     Wp = Us * inv(Up);
    ...
    Us = (1 - rho) * Us + rho * Alphas * C' * inv(C * C' + par.nu * eye(size(Alphas, 1))); % �����е�Us^{-1}
    Up = (1 - rho) * Up + rho * Alphap * C' * inv(C * C' + par.nu * eye(size(Alphap, 1))); % �����е�Up^{-1}
    Ws = Up * inv(Us); 
    Wp = Us * inv(Up);
